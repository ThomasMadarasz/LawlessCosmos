...
func _instantiate_enemies(wave_index : int) -> Array[EnemyCharacter]:
	...
	for i: int in 3:
		var enemy_resource: EnemyCharacterResource = null
		match i:
			0:
		...
		var enemy_character: EnemyCharacter = instantiate_enemy(enemy_resource, i)
...
func instantiate_enemy(enemy_resource : EnemyCharacterResource, position_id: int) -> EnemyCharacter:
	var enemy_prefab: PackedScene = ResourceLoader.load_threaded_get(enemy_resource.file_path)
	var enemy_character: EnemyCharacter = enemy_prefab.instantiate() as EnemyCharacter
