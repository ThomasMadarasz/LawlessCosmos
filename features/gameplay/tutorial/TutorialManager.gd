...
func start_tutorial(is_tutorial: bool = false) -> void:
	if is_tutorial:
		_previous_button.disabled = true
		_connect_signals()
	else:
		for n: TutorialElement in tuto_elements:
			var tuto_tab_prefab: Resource = load(n.tab_prefab_path)
			var tuto_tab: Variant = tuto_tab_prefab.instantiate() #TODO: Find the type
...
func _on_card_discarded(_card: Card, _is_played: bool = false) -> void:
...
func _on_status_added(_status: Variant, _caster: Character) -> void:
...
func _check_tuto_element(type : TutorialElement.TutoType) -> void:
	for i: int in tuto_elements.size():
		if tuto_elements[i].is_available or not tuto_elements[i].type.has(type): 
			continue
		var is_ready: bool = true
		for n: TutorialElement in tuto_elements[i].previous_elements:
		...
		var tuto_tab_prefab: Resource = load(tuto_elements[i].tab_prefab_path)
		var tuto_tab: Variant = tuto_tab_prefab.instantiate() #TODO: Find this type
		_tab_container.add_child(tuto_tab)
		if not visible:
			show()
