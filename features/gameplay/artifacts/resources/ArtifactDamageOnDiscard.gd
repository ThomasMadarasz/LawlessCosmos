...
func _on_card_discarded(card: Card, _hand: Hand, _is_played : bool) -> void:
	var damage_amount: int = card.card_resource.value
	var potential_targets: Array[Character] = CharactersManager.enemy_characters if owner_character is PlayerCharacter else CharactersManager.player_characters
