...
var _current_slot_target: int

var efficiency: int:
	get:
		if efficiency == 0 and _efficiency_range.x > 0:
			var multiplier: float = BattleStageManager.difficulty_multiplier if BattleStageManager.current_wave_data.is_boss_wave else BattleStageManager.difficulty_multiplier * BattleStageManager.wave_efficiency_multiplier
...
func target_random_player_character() -> Array[int]:
	if _current_slot_target == -1: _current_slot_target = CharactersManager.get_player_filled_slots().pick_random()
...
func target_lowest_player() -> Array[int]:
	var lowest_player: Character = null
...
func target_highest_player() -> Array[int]:
	if _current_slot_target == -1:
		var highest_player: Character = null
		for n in CharactersManager.player_characters:
			if n == null: continue
...
func target_lowest_health_enemy() -> Array[int]:
	var lowest_enemy: Character = null
...
func target_most_cards_player() -> Array[int]:
	if _current_slot_target == -1:
		var most_cards_player: Character = null
		for n in CharactersManager.player_characters:
			if n == null: continue
...
func target_front_row_ally_at_start() -> Array[int]:
	var target: Character = owner_character.enemies_by_position_at_start[0] if not owner_character.enemies_by_position_at_start[1].is_dead else null
	if target == null:
...
func target_mid_row_ally_at_start() -> Array[int]:
	var target: Character = owner_character.enemies_by_position_at_start[1] if not owner_character.enemies_by_position_at_start[1].is_dead else null
	if target == null:
		target = CharactersManager.enemy_characters.pick_random()
	return [target.position_id]

func target_back_row_ally_at_start() -> Array[int]:
	var target: Character = owner_character.enemies_by_position_at_start[2] if not owner_character.enemies_by_position_at_start[1].is_dead else null
	if target == null:
		target = CharactersManager.enemy_characters.pick_random()
	return [target.position_id]

func target_most_bleed() -> Array[int]:
	var target : Character = CharactersManager.player_characters.pick_random() if _current_slot_target == null else CharactersManager.players_positions_by_id[_current_slot_target]
...
func target_most_poison() -> Array[int]:
	var target : Character = CharactersManager.player_characters.pick_random() if _current_slot_target == null else CharactersManager.players_positions_by_id[_current_slot_target]
	var current_top_poison : int = target.status.get_status_count("poison")
	for n in CharactersManager.player_characters:
		if n.status.get_status_count("poison") > current_top_poison:
			target = n
	_current_slot_target = target.position_id
	return[_current_slot_target]
