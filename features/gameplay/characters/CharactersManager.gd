...
var is_a_character_performing_move: bool = false
var is_a_hand_manipulation_enabled: bool = false

var is_wave_dead: bool = false
...
func _on_character_death(character: Character) -> void:
	if character is EnemyCharacter:
		...
		var enemy_index: int = enemies_positions_by_id.find(character)
		print(character)
		if enemy_index == -1:
			print(character)
			print("error")
...
func instantiate_player_character(i : int) -> void:
	var player_prefab: Resource = null
	...
	var player_character: PlayerCharacter = player_prefab.instantiate() as PlayerCharacter
...
func clear_dead_enemy_characters() -> void:
	for n: Character in dead_enemy_characters:
		n.queue_free()
...
func set_selected_character(character: Character, force_switch: bool = false) -> void:
...
func hide_slots(only_targeted : bool = false) -> void:
	for n: Slot in BattleStageManager.level_manager.player_characters_positions + BattleStageManager.level_manager.enemy_characters_positions:
		n.set_targeted(false)
		if not only_targeted: 
			n.set_targetable(Color.WHITE, false)

func get_player_filled_slots() -> Array[int]:
	var slots_id: Array[int] = [] as Array[int]
	player_characters.sort_custom(sort_by_position)
	for n: Character in player_characters:
...
func get_enemy_filled_slots() -> Array[int]:
	...
	for n: Character in enemy_characters:
		slots_id.push_back(n.position_id)
	return slots_id
...
func sort_by_position(a: Character, b: Character) -> bool:
	if a.position_id < b.position_id:
		return true
	return false

func order_enemies_positions() -> void:
	for i: int in enemies_positions_by_id.size():
...
func order_players_positions() -> void:
	for i: int in players_positions_by_id.size():
		if players_positions_by_id[i]== null: continue
		players_positions_by_id[i].position_id = i
	for n: Character in enemies_positions_by_id:
		if not n == null:
			n.set_target()
...
func move_characters_to_their_respective_slots(slots_positions : Array[Slot], characters_order : Array[Character]) -> void:
	for i: int in slots_positions.size():
		var character: Character = characters_order[i]
...
func order_characters(characters_order : Array[Character], is_updating_hand_positions: bool) -> void:
	for i: int in characters_order.size():
...
func get_next_player_by_position(set_as_selected: bool) -> PlayerCharacter:
	var wanted_id: int = selected_character.position_id + 1
	if wanted_id >= player_characters.size(): wanted_id = 0
	var player_character: Character = player_characters[wanted_id]
