...
func gain_move_points() -> void:
	if is_dead: return
	on_poison_tick_phase.emit()
	if is_dead: return
	if status.check_for_paralysis(): return
	update_move_points(character_resource.hand_data.reload_size)
	on_acceleration_tick_phase.emit()
...
func _update_cards() -> void:
	var cards: Array[Node] = cards_parent.get_children().duplicate() #NOTE: The duplicqte dpn't allow to keep the array type
	cards.reverse()
...
func perform_move() -> void:
	...
	var target: Character = get_current_target()
...
func show_targets(_id: int = 0) -> void:
	var targeted_slots: Array[int] = current_move.move_targets.get_targeted_slots(current_move)
	var positions: Array[Slot] = BattleStageManager.level_manager.player_characters_positions if current_move.is_move_targeting_an_opponent() else BattleStageManager.level_manager.enemy_characters_positions
	for n: int in targeted_slots:
		var slot: Slot = positions[n] as Slot
		slot.set_targetable(current_move.color)
		slot.set_targeted()

func get_current_target() -> Character:
	var targeted_slots: Array[int] = current_move.move_targets.get_targeted_slots(current_move)
	var target: Character = CharactersManager.get_player_character_with_position_id(targeted_slots[0]) as Character if current_move.is_move_targeting_an_opponent() else CharactersManager.get_enemy_character_with_position_id(targeted_slots[0]) as Character
	return target
