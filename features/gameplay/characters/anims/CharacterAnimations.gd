...
func set_idle() -> void:
	var idle_speed: float = randf_range(_idle_data.idle_duration_min, _idle_data.idle_duration_max)
	if not _idle_data.idle_scale_ratio == 1:
		var full_size_y: float = _animated_sprite_scale.y
		var min_size_y: float = full_size_y * _idle_data.idle_scale_ratio
	...
	if not _idle_data.idle_floating_displacement_y == 0:
		var base_pos_y: float = _animated_sprite_base_position.y - (_idle_data.idle_floating_displacement_y/2)
	...
	if not _idle_data.idle_shadow_scale_ratio == 1:
		var full_size: Vector2 = _shadow_scale
		var min_size: Vector2 = full_size * _idle_data.idle_shadow_scale_ratio
...
func move(move_anim_data: MoveAnimData) -> void:
	kill_idle()
	var base_pos_x: float = _animated_sprite_base_position.x
