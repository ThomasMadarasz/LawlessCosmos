...
func show_efficiency_preview(move: Move, performer: Character = CharactersManager.selected_character)  -> void:
	var power: float = performer.hand.current_final_hand.power if performer is PlayerCharacter else performer.current_move.enemy_power
	var value: int = move.calculate_power_with_damage_multiplier(power * move.move_power_multiplier, owner_character)
	var hit_amount: int = move.get_hit_amount()
...
func update_shield(resource: CharacterResource = owner_character.character_resource) -> void:
	var tween_shield: Tween = create_tween()
...
func update_health(resource: CharacterResource = owner_character.character_resource) -> void:
	var health_start_value: int = _current_displayed_health_value
	...
	var tween_health_bar: Tween = create_tween()
...
func count_health_label(given_number: float, label: Label) -> void:
	var rounded_number: int = roundi(given_number)
	label.text = str(rounded_number) + "/" + str(owner_character.max_health)

func count_shield_label(given_number: float, label: Label) -> void:
	var rounded_number: int = roundi(given_number)
	label.text = str(rounded_number)

func count_health_bar(given_number: float) -> void:
