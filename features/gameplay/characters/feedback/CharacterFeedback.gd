...
func play_damage_feedback(health_lost: int) -> void:
	...
	var damage_vfx: CPUParticles2D = _set_up_particles(ResourcesManager.damage_vfx)
...
func play_shield_feedback(current_shield_amount_received: int) -> void:
	var shield_vfx: CPUParticles2D = _set_up_particles(ResourcesManager.shield_vfx)
...
func play_shield_lost_feedback(value : int) -> void:
	if value > 0: 
		var shield_damage_vfx: CPUParticles2D = _set_up_particles(ResourcesManager.shield_damage_vfx)
...
func play_heal_feedback(heal_received: int) -> void: 
	var heal_vfx: CPUParticles2D = _set_up_particles(ResourcesManager.heal_vfx)
...
func play_status_application_feedback(status_id: StringName) -> void:
	#DEPRECATED TODO
	match status_id:
		"poison":
...
func play_status_trigger_feedback(status_id: StringName) -> void:
	#DEPRECATED TODO
	match status_id:
		"poison":
			_play_poison_particles()
...
func _play_poison_particles() -> void:
	var poison_particles: CPUParticles2D = _set_up_particles(ResourcesManager.poison_vfx)
...
func _shake(time: float, shake_data: ShakeData) -> void:
	var frequency_weight: float = ease(time, shake_data.frequency_curve)
	var frequency: float = lerpf(shake_data.frequency_start, shake_data.frequency_end, frequency_weight)
	if _shake_time < frequency: return
	var decay_weight: float = ease(time, shake_data.decay_curve)
	var decay: float = lerpf(shake_data.decay_start, shake_data.decay_end, decay_weight)
	if not frequency == 0: _shake_time = fmod(_shake_time, frequency) 
	var amplitude: float = shake_data.amplitude * decay
...
func _set_up_particles(particles_resource: PackedScene) -> CPUParticles2D:
	var particles: CPUParticles2D = particles_resource.instantiate() as CPUParticles2D
	add_child(particles)
