...
var suit_amount_bonus: Dictionary = {0:0, 1:0, 2:0, 3:0}
...
func _on_characters_manager_initialized() -> void:
	var moves_textures: Array = [null, null, null, null]
	for n: int in character_resource.current_moves.size():
...
func choose_target(character: Character) -> void:
	var move: Move = character_resource.current_moves[hand.current_final_hand.chosen_suit]
...
func select_suit(suit_id : int) -> void:
	...
	var available_targets: Dictionary = move.get_allowed_targets()
	for n: Character in available_targets.keys():
...
func stop_target_choice(is_canceled: bool = true) -> void:
	is_choosing_target_to_perform_move = false
	CharactersManager.hide_slots()
	for n: MoveButton in hand.ui.moves_ui.moves_buttons:
		n.set_selected(false)
	for n: Character in CharactersManager.player_characters + CharactersManager.enemy_characters:
...
func add_artifact(artifact_resource : ArtifactResource) -> void:
	var current_stack: int = 1 if not artifacts.keys().has(artifact_resource) else artifacts[artifact_resource] + 1
	artifacts[artifact_resource] = current_stack
	artifact_resource.enable(self)

func show_targets(id: int = 0) -> void:
	var move: Move = character_resource.current_moves[id]
	var available_targets: Dictionary = move.get_allowed_targets()
	for n: Array[Character] in available_targets.values():
		for character: Character in n:
			var slot: Slot = _level_manager.player_characters_positions[character.position_id] if character is PlayerCharacter else _level_manager.enemy_characters_positions[character.position_id] as Slot
