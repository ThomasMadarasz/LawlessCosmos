...
func register_rewards_manager(manager: RewardsManager) -> void:
	_rewards_manager = manager
	_rewards_manager.on_reward_assigned.connect(_on_reward_assigned)
	for n: RewardCharacterButton in _rewards_manager.characters_buttons:
...
func draft(probabilities: Dictionary) -> void:
	var random_value: int = randi() % 100
	var cumulative: int = 0
	for key: Reward in probabilities.keys():
