...
func perform() -> void:
	for n: int in _targets.size():
		...
		var ally_target: Character = _get_lowest_health_ally(_owner_character)
		var efficiency_percentage: float = _calculate_health_difference_percentage(target, ally_target)
...
func _get_lowest_health_ally(owner_character: Character) -> Character:
	...
	var lowest_health_character: Character = owner_character
...
func _calculate_health_difference_percentage(target_1 : Character, target_2 : Character) -> float:
	var target_1_percentage: float = float(target_1.character_resource.current_health) / float(target_1.max_health)
	var target_2_percentage: float = float(target_2.character_resource.current_health) / float(target_2.max_health)
