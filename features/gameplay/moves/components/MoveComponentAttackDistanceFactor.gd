...
func perform() -> void:
	...
	for n: int in _targets.size():
		...
		var damage_multiplier: float = 1.0 - ((_owner_character.position_id + target.position_id) * _distance_factor)
		var results: Array[Variant] = _move.attack_target(_move.final_power * damage_multiplier, target, not _move.current_targets_count == 1)
