...
func solve(targets : Array[Character]) -> void:
	...
	var remaining_components: Array[MoveComponent] = _move_components.duplicate(false)
	for n: MoveComponent in _move_components:
		n.set_move(_move)
		...
		if n is MoveComponentMultiHits:
			for hit: int in n.get_hit_amount():
...
func duplicate() -> MoveSolver:
	var new_move_solver: MoveSolver = MoveSolver.new(_move)
