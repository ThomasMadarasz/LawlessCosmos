...
@export var move_power_multiplier: float = 1.0
...
var current_slot_target: Variant #TODO: Check what type this is

var final_power: int:
	get: return get_final_power()


const PERCENTAGE_DIVIDER: float = 100.0
const MAX_CARDS_IN_COMBINATION: int = 5

var _allowed_targets_callables : Dictionary = { # int:lambda->int:Character
...
func init() -> void:
	...
	for n: MoveComponent in components:
		n.set_move(self)
...
func perform(target : Character) -> void:
	...
	var move_solver: MoveSolver = MoveSolver.new(self)
	await move_solver.solve(get_targets_from_target(target))
	on_move_logic_ended.emit(self)


func attack_target(power: float, target : Character, is_group_attack: bool = false) -> Array[Variant]:
	if not is_group_attack:
		var characters: Array[Character] = CharactersManager.players_positions_by_id if target is PlayerCharacter else CharactersManager.enemies_positions_by_id
		for n: Character in characters:
			if n == null: continue
	...
	var health_lost: int = target.take_damage(roundi(power), owner_character)
...
func calculate_power_with_damage_multiplier(power: float, target : Character = null) -> int:
	var allowed_targets_dic: Dictionary = get_allowed_targets()
	if not target == null and not allowed_targets_dic.keys().has(target):
		target = null
	for n: DamageMultiplierEvaluator in owner_character.inflicted_damage_multiplier:
		power *= n.get_damage_multiplier(owner_character, target)
	if not target == null:
		for n: DamageMultiplierEvaluator in target.received_damage_multiplier:
			power *= n.get_damage_multiplier(target)
	return roundi(power)

func get_hit_amount() -> int: #NOTE: Always 1 ?
	return 1

func get_targets_from_target(target: Character) -> Array[Character]:
	var allowed_targets_dic: Dictionary = get_allowed_targets()
	var targets: Array[Character] = [] as Array[Character]
	for n: Character in allowed_targets_dic[target]:
		targets.push_back(n)
...
func is_target_valid(target: Character) -> bool:
	var allowed_targets: Dictionary = get_allowed_targets()
...
func get_allowed_targets() -> Dictionary: # int:Character
...
func get_any_opponent() -> Dictionary:
	...
	for i: int in range(0,3):
		var character_key: Character = CharactersManager.get_player_character_with_position_id(i) as Character if owner_character is EnemyCharacter else CharactersManager.get_enemy_character_with_position_id(i) as Character
...
func get_farthest_opponent() -> Dictionary:
	...
	for i: int in range(2, -1, -1):
		var character_key: Character = CharactersManager.get_enemy_character_with_position_id(i) as Character if owner_character is PlayerCharacter else CharactersManager.get_player_character_with_position_id(i) as Character
		if not character_key == null: 
			targets[character_key] = [character_key] as Array[Character]
			break
	return targets
...
func get_all_opponents() -> Dictionary:
	var targets: Dictionary = {}
	var characters: Array[Character] = CharactersManager.enemy_characters if owner_character is PlayerCharacter else CharactersManager.player_characters
	for n: Character in characters:
...
func get_any_ally() -> Dictionary:
	var targets: Dictionary = {}
	for i: int in range(0,3):
		var character_key: Character = CharactersManager.get_player_character_with_position_id(i) as Character if owner_character is PlayerCharacter else CharactersManager.get_enemy_character_with_position_id(i) as Character
		if not character_key == null: 
			targets[character_key] = [character_key] as Array[Character]
	return targets
...
func get_all_allies() -> Dictionary:
	var targets: Dictionary = {}
	var characters: Array[Character] = CharactersManager.player_characters if owner_character is PlayerCharacter else CharactersManager.enemy_characters
	for n: Character in characters:
		targets[n] = characters
	return targets

func is_move_targeting_an_opponent() -> bool:
	var value: bool = not (_allowed_targets == AllowedTargets.SELF or _allowed_targets == AllowedTargets.ANY_ALLY or _allowed_targets == AllowedTargets.ALL_ALLIES )
	return value
...
func _get_formatted_description() -> String:
	var new_formated_description: String = super._get_formatted_description()
	if iteration > 0:
...
func get_description(current_description: String, suit_amount: int, power: int) -> String:
	var description_to_return: String = current_description
