...
func initialize_colors(color: Color) -> void:
	...
	var transparent_color: Color = Color(color.r, color.g, color.b, 5.0/255.0)
...
func _on_card_drawn(_card: Card = null) -> void:
	update_hand_labels()

func _on_card_discarded(_card: Card = null, _is_played: bool = false) -> void:
	update_hand_labels()
...
func set_moves_descriptions() -> void:
	for i: int in 4:
		var move: Move = owner_character.character_resource.current_moves[i]
...
func sort_counting_and_ascending(a: CardResource, b: CardResource) -> bool:
	if current_final_hand.hand_ranking == Enums.HandRankings.FULL_HOUSE or current_final_hand.hand_ranking == Enums.HandRankings.FULL_HOUSE_FLUSH:
		...
		for n: CardResource in current_final_hand.cards_resources:
