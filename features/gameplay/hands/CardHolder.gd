...
const _MAX_HAND_SIZE: float = 600.0
const _MIN_HAND_SIZE: float = 460.0
...
func on_player_won_wave() -> void:
	var hand_size: float = _MIN_HAND_SIZE 
...
func _set_cards_position_and_rotation_spots() -> void:
	...
	for i: int in range(hand.current_cards_order.size()):
...
func update_cards_positions(are_positions_and_rotations_to_update : bool = false) -> void:
	...
	for n: int  in hand.current_cards_order.size(): 
		...
		var destination: Vector2 = hand.current_cards_order_position[n] - _cards_length_offset + hand.global_position + (_destination_size/2)
...
func set_destination_size_and_pos() -> void:
	var hand_size: float = _MIN_HAND_SIZE if not CharactersManager.selected_character == owner_character else _MAX_HAND_SIZE + Card.CARD_WIDTH
	_destination_size = Vector2(hand_size, hand.hand_container.size.y)
...
func update_cards_positions_on_card_dragged(dragged_card: Card) -> void:
	if abs(dragged_card.global_position.y - hand.global_position.y) <= _y_drag_detection_distance:
		var current_pos_order: int = hand.current_cards_order.find(dragged_card)
		var closest_pos_order: int = hand.current_cards_order.find(dragged_card)
		var current_card_distance: float = abs(hand.current_cards_order_position[closest_pos_order].x - _cards_length_offset.x + hand.global_position.x + (_destination_size/2).x - dragged_card.global_position.x)
		for n: int in hand.current_cards_order_position.size():
			var challenged_card_distance: float = abs(hand.current_cards_order_position[n].x - _cards_length_offset.x + hand.global_position.x + (_destination_size/2).x - dragged_card.global_position.x)
