...
var current_cards: Dictionary = {} # Card:CardResource
var current_cards_order_position: Dictionary = {} # int : Vector2
...
func _on_new_turn() -> void:
	if not owner_character.status.check_for_paralysis():
		var cards_amount: int = owner_character.character_resource.hand_data.reload_size
		if not current_cards.size() >= owner_character.character_resource.hand_data.draw_limit:
			draw_cards(cards_amount)
	owner_character.on_acceleration_tick_phase.emit()
...
func draw_cards(amount: int) -> Array[Card]:
	var drawn_cards: Array[Card] = [] as Array[Card]
	for i: int in amount:
		var n: CardResource = deck.draw_cards_resources(1)[0]
		var card: Card = _instantiate_card(n)
...
func draw_specific_card(card_resource: CardResource, draw_position: Vector2 = BattleStageManager.current_deck.position) -> Card:
	var card: Card = _instantiate_card(card_resource)
	card.global_position = draw_position
	return card

func _instantiate_card(card_resource : CardResource) -> Card:
	var instantiated_card: Card = deck.display_card(card_resource, cards_holder)
...
func fill_hand(is_initial_draw: bool = false) -> void:
	var card_amount: int = owner_character.hand_data.initial_draw_amount if is_initial_draw else owner_character.hand_data.draw_limit
...
func discard_card(discarded_card: Card, is_played: bool = true) -> void:
...
func discard_cards(cards: Array[Card], is_played: bool = true) -> void:
	var discarded_cards: Array[Card] = cards.duplicate(true)
	for n: Card in discarded_cards:
		discard_card(n, is_played)

func discard_all_cards(is_played: bool = true) -> void:
	var cards_to_discard: Array[Card] = current_cards_order
	if cards_to_discard.size() > 0 : discard_cards(cards_to_discard, is_played)
	current_selected_cards.clear()

func discard_selected_cards(is_played: bool = true) -> void:
...
func show_hand_full_pop_up() -> void:
	...
	var tween: Tween = get_tree().create_tween().set_trans(Tween.TRANS_CIRC).set_ease(Tween.EASE_IN)
...
func calculate_current_final_hand() -> FinalHandData:
	...
	for n: Card in current_selected_cards: 
...
func is_hand_full(play_feedback: bool) -> bool:
	var is_hand_full_value: bool = current_cards.size() >= owner_character.character_resource.hand_data.max_hand_size
	if play_feedback and is_hand_full_value:
		show_hand_full_pop_up()
	return is_hand_full_value

func sort_descending_suits(a: Card, b: Card) -> bool:
...
func sort_ascending_card_value(a: Card, b: Card) -> bool:
