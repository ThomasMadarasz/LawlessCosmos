...
const _CARD_PREFAB: Resource = preload("res://datas/prefabs/cards/p_card.tscn")
...
const _SPACE_HEIGHT: int = 150
const _SPACE_WIDTH: int = 100
const _MARGIN: int = 125
const _HOVER_TIME: float = 1.0
...
func _on_mouse_exited_pile() -> void:
	...
	if _is_deck_opened or _is_discard_pile_opened:
		_cards_panel.hide()
		for n: Card in _displayed_cards:
			n.reset()
...
func draw_cards_resources(amount: int) -> Array[CardResource]:
	...
	if deck_resource.cards_resources.size() <= 0:
		shuffle_cards(true)
	for i: int in amount:
		var card_resource: CardResource = deck_resource.cards_resources[0]
...
func remove_specific_cards_resources(cards_resources : Array[CardResource]) -> void:
	for n: CardResource in cards_resources:
		if deck_resource.cards_resources.size() <= 0:
			shuffle_cards(true)
...
func _show_cards(cards: Array[CardResource], start_position: Vector2) -> void:
	_cards_panel.show()
	var duplicated_card_resources: Array[CardResource] = cards.duplicate()
	for n: CardResource in duplicated_card_resources:
		if not is_instance_valid(n):
			duplicated_card_resources.erase(n)
	duplicated_card_resources.sort_custom(sort_cards)
	var array_of_suit_arrays_of_cards: Array[Array] = [[], [], [], []]
	_displayed_cards = []
	for n: CardResource in duplicated_card_resources:
		var card_instance: Card = display_card(n, _cards_parent, start_position)
		array_of_suit_arrays_of_cards[n.suit].push_back(card_instance)
		_displayed_cards.push_back(card_instance)
	for i: int in array_of_suit_arrays_of_cards.size():
		for j: int in array_of_suit_arrays_of_cards[i].size():
...
func sort_cards(a: CardResource, b: CardResource) -> bool:
	if a.suit < b.suit:
		return true
	elif a.suit == b.suit:
