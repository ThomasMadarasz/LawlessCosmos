...
func get_final_hand(cards_resources: Array[CardResource], cards: Dictionary, character: Character) -> FinalHandData:
	var final_hand: FinalHandData = FinalHandData.new(character)
...
func _calculate_ranking(final_hand: FinalHandData) -> Enums.HandRankings:
	var amount_of_values: Dictionary = {}
	final_hand.suits_amount = {0:0, 1:0 , 2:0, 3:0}
	var highest_card: CardResource = null
	var lowest_card: CardResource = null
	var has_an_ace: bool = false
	for n: CardResource in final_hand.cards_resources:
	...
	for n: int in final_hand.suits_amount.values():
		if n == 5: #TODO: Remove magic number
			is_flush = true
			break
	
	for n: int in amount_of_values.keys():
		if amount_of_values[n] == 5:
			for card_resource: CardResource in final_hand.cards_resources: final_hand.counting_cards[card_resource] = true
			...
			for card_resource: CardResource in final_hand.cards_resources: final_hand.counting_cards[card_resource] = true if card_resource.value_id == n else false
	...
	if amount_of_values.values().count(1) == 5:
		...
		var highest_value: int = highest_card.value_id if not highest_card.value_id == 0 else 13
		if highest_value - lowest_card.value_id == 4: is_straight = true
		
		if is_straight:
			if highest_value == 13: #TODO:  Remove magic number
				for card_resource: CardResource in final_hand.cards_resources: final_hand.counting_cards[card_resource] = true
	...
	if amount_of_values.values().count(2) == 2: 
		for card_resource: CardResource in final_hand.cards_resources: final_hand.counting_cards[card_resource] = true if amount_of_values[card_resource.value_id] == 2 else false
		return Enums.HandRankings.TWO_PAIR
	elif amount_of_values.values().count(2) == 1: 
		for card_resource: CardResource in final_hand.cards_resources: final_hand.counting_cards[card_resource] = true if amount_of_values[card_resource.value_id] == 2 else false
		return Enums.HandRankings.PAIR
	else:
		for n: int in final_hand.cards_resources.size(): final_hand.counting_cards[final_hand.cards_resources[n]] = true if n == 0 else false
...
func _calculate_available_suits(final_hand: FinalHandData) -> Array[Enums.Suits]:
	...
	final_hand.suits_amount = {0:0, 1:0 , 2:0, 3:0}
	for n: CardResource in final_hand.counting_cards.keys():
		if final_hand.counting_cards[n] == true:
			final_hand.suits_amount[n.suit] += 1
	for n: int in final_hand.suits_amount:
