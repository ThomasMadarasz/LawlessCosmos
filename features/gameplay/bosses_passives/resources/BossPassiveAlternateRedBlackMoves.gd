...
func _disable_suits_moves() -> void:
	var suits_to_disable: Array[int] = _get_suits_to_disable()
	for n: Character in CharactersManager.player_characters:
		for move: Move in n.character_resource.current_moves:
...
func _set_defense_form() -> void:
	...
	owner_character.status.add_status("damage_immune", _damage_immune_amount_on_defense_form, owner_character)
