...
func switch_hands(character: Character) -> void:
	...
	var character_1_cards: Array = _character_1.hand.current_cards.keys().duplicate()
	var character_2_cards: Array = _character_2.hand.current_cards.keys().duplicate()
	for n: Card in character_1_cards:
	...
	for n: Card in character_2_cards:
...
func disable_hand_manipulation() -> void:
	_character_1 = null
	_character_2 = null
	for n: Dictionary in InputManager.on_character_clicked.get_connections():
			if n["callable"] == _on_character_clicked: InputManager.on_character_clicked.disconnect(_on_character_clicked)
	for n: Dictionary in InputManager.on_hand_clicked.get_connections():
