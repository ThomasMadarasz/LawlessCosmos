...
func _on_card_clicked(card: Card) -> void:
	...
	var card_index: int = card.owner_hand.current_cards_order.find(card)
	if _is_modifying_previous_card and card_index < card.owner_hand.current_cards_order.size() - 1:
		var previous_card: Card = card.owner_hand.current_cards_order[card_index + 1]
		_modify_value(previous_card, card.card_resource.value_id - 1)
		is_valid = true
	if _is_modifying_next_card and card_index > 0:
		var next_card: Card = card.owner_hand.current_cards_order[card_index - 1]
