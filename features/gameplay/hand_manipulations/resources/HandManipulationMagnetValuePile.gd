...
func _on_card_clicked(card: Card) -> void:
	var cards_resources_list: Array[CardResource] = BattleStageManager.current_deck.deck_resource.discarded_cards_resources if _is_discard_pile else BattleStageManager.current_deck.deck_resource.cards_resources
	var pile_label: Label = BattleStageManager.current_deck.discard_pile_label if _is_discard_pile else BattleStageManager.current_deck.deck_label
	var cards_to_attract: Array[CardResource] = [] #NOTE: Sometimes cards_to_attract is used for cards and sometimes for CardResource it's confusing
	for card_resource: CardResource in cards_resources_list:
		if card_resource.value_id == card.card_resource.value_id:
			cards_to_attract.push_back(card_resource)
			if _is_only_first_card:
				break
	if cards_to_attract.is_empty():
		#TODO: Add feedback that there is no cards with this value in pile
	...
	if card.owner_hand.current_cards.size() + cards_to_attract.size() > card.owner_hand.owner_character.character_resource.hand_data.max_hand_size:
		...
		disable_hand_manipulation()
		return
	for n: CardResource in cards_to_attract:
...
func disable_hand_manipulation() -> void:
	for n: Dictionary in InputManager.on_card_clicked.get_connections():
