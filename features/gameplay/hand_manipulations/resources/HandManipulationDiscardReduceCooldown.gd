...
func _on_card_clicked(card: Card) -> void:
	...
	var hand: Hand = card.owner_hand
	if not card.is_locked:
		card.is_discarding = true
		hand.discard_card(card, false)
		var eligible_manipulations: Array[HandManipulation] = []
		for n: HandManipulation in manager.hand_manipulations:
		...
		if eligible_manipulations.size() > 0:
			var manipulation_to_reduce: HandManipulation = eligible_manipulations.pick_random()
