...
func _on_card_clicked(card: Card) -> void:
	var hands_to_search: Array[Hand] = []
	for n: Character in CharactersManager.player_characters:
		if not n.hand == card.owner_hand:
			hands_to_search.push_back(n.hand)
	var cards_to_attract: Array[Card]
	for hand: Hand in hands_to_search:
		for n: Card in hand.current_cards.keys():
	...
	if card.owner_hand.current_cards.size() + cards_to_attract.size() > card.owner_hand.owner_character.character_resource.hand_data.max_hand_size:
		...
		return
	for n: Card in cards_to_attract:
		var ex_owner_hand: Hand = n.owner_hand
		var new_owner_hand: Hand = card.owner_hand
