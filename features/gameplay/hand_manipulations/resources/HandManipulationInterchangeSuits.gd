extends HandManipulationResource

class_name HandManipulationInterchangeSuits

var _suits_keys_values: Dictionary = { Enums.Suits.SPADE : Enums.Suits.CLUB, Enums.Suits.CLUB : Enums.Suits.SPADE, Enums.Suits.HEART : Enums.Suits.DIAMOND, Enums.Suits.DIAMOND : Enums.Suits.HEART}

func activate_hand_manipulation() -> void:
	super.activate_hand_manipulation()
	InputManager.on_card_clicked.connect(_on_card_clicked)

func _on_card_clicked(card: Card) -> void:
	var targeted_suit: Enums.Suits = _suits_keys_values[card.card_resource.suit]
	if _change_suit(card, targeted_suit):
		CharactersManager.set_selected_character(card.owner_hand.owner_character)
		super.use_hand_manipulation()
	else:
		disable_hand_manipulation()

func disable_hand_manipulation() -> void:
	for n: Dictionary in InputManager.on_card_clicked.get_connections():
		if n["callable"] == _on_card_clicked: InputManager.on_card_clicked.disconnect(_on_card_clicked)
	super.disable_hand_manipulation()
