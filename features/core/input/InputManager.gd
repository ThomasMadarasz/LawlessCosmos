...
var move_targetable_characters: Dictionary = {} #Character : Array[Character]
...
func _set_hit_obj() -> void:
	var new_obj: Node = null
	if _results.size() < 1: 
		_hit_obj = new_obj
		return
	if not is_dragging:
		var collider_parent: Node = null
		for i: int in _results.size():
			var parent: Node = _results[i].collider.get_parent()
...
func _handle_mouse_character_move_preview() -> void:
	if _hit_obj == null:
		...
		for n: Character in move_targetable_characters.keys():
	...
	if not CharactersManager.selected_character == null and CharactersManager.selected_character is PlayerCharacter and _hit_obj is Character and move_targetable_characters.has(_hit_obj):
		for n: Character in move_targetable_characters[_hit_obj]:
			_current_move_target = n
			_current_move_target.feedback.play_target_feedback(true)
			var slot: Slot = BattleStageManager.level_manager.player_characters_positions[_current_move_target.position_id] if _current_move_target is PlayerCharacter else BattleStageManager.level_manager.enemy_characters_positions[_current_move_target.position_id]
...
func _drop_card() -> void:
	...
	var target_hand: Hand = null
	for n: Dictionary in _results:
		var n_collider_parent: Node = n.collider.get_parent()
...
func _send_click_event(current_hit_obj: Variant) -> void:
...
func _check_for_dragged_card(current_hit_obj: Node) -> void:
...
func _handle_power_previews() -> void:
	if _is_hit_obj_new:
		for n: Character in CharactersManager.characters:
			n.ui.hide_efficiency_preview()
		if _hit_obj is Character:
			var move: Move = CharactersManager.selected_character.get_current_move()
			var allowed_targets: Dictionary = move.get_allowed_targets()
			if allowed_targets.has(_hit_obj):
				for n: Character in allowed_targets[_hit_obj]:
